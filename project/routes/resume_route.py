...
def create_resume():
    ...
    try:
        ...
        return jsonify({
            "message": f"Error create resume {str(e)}"
        }), 500


@resume_router.route('/resumes', methods=["DELETE"])
@jwt_required()
def delete_resume():
    """Удаление резюме"""
    try:
        ...
        return jsonify({
            "message": f"Error delete resume {str(e)}"
        }), 500
...
def update_resume():
    ...
    try:
        ...
        return jsonify({
            "message": f"Error update resume {str(e)}"
        }), 500
...
def get_user_resumes():
    ...
    try:
        ...
        return jsonify({
            "message": f"Error get user resume {str(e)}"
        }), 500